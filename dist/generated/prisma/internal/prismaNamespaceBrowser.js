/* !!! This is code generated by Prisma. Do not edit directly. !!! */
/* eslint-disable */
// biome-ignore-all lint: generated file
// @ts-nocheck 
/*
 * WARNING: This is an internal file that is subject to change!
 *
 * 🛑 Under no circumstances should you import this file directly! 🛑
 *
 * All exports from this file are wrapped under a `Prisma` namespace object in the browser.ts file.
 * While this enables partial backward compatibility, it is not part of the stable public API.
 *
 * If you are looking for your Models, Enums, and Input Types, please import them from the respective
 * model files in the `model` directory!
 */
import * as runtime from "@prisma/client/runtime/index-browser";
export const Decimal = runtime.Decimal;
export const NullTypes = {
    DbNull: runtime.NullTypes.DbNull,
    JsonNull: runtime.NullTypes.JsonNull,
    AnyNull: runtime.NullTypes.AnyNull,
};
/**
 * Helper for filtering JSON entries that have `null` on the database (empty on the db)
 *
 * @see https://www.prisma.io/docs/concepts/components/prisma-client/working-with-fields/working-with-json-fields#filtering-on-a-json-field
 */
export const DbNull = runtime.DbNull;
/**
 * Helper for filtering JSON entries that have JSON `null` values (not empty on the db)
 *
 * @see https://www.prisma.io/docs/concepts/components/prisma-client/working-with-fields/working-with-json-fields#filtering-on-a-json-field
 */
export const JsonNull = runtime.JsonNull;
/**
 * Helper for filtering JSON entries that are `Prisma.DbNull` or `Prisma.JsonNull`
 *
 * @see https://www.prisma.io/docs/concepts/components/prisma-client/working-with-fields/working-with-json-fields#filtering-on-a-json-field
 */
export const AnyNull = runtime.AnyNull;
export const ModelName = {
    User: 'User',
    Company: 'Company',
    Jobs: 'Jobs',
    Instructions: 'Instructions',
    Session: 'Session'
};
/*
 * Enums
 */
export const TransactionIsolationLevel = runtime.makeStrictEnum({
    ReadUncommitted: 'ReadUncommitted',
    ReadCommitted: 'ReadCommitted',
    RepeatableRead: 'RepeatableRead',
    Serializable: 'Serializable'
});
export const UserScalarFieldEnum = {
    id: 'id',
    firstName: 'firstName',
    lastName: 'lastName',
    password: 'password',
    confirmPassword: 'confirmPassword',
    location: 'location',
    email: 'email',
    phoneNumber: 'phoneNumber',
    linkedInURL: 'linkedInURL',
    githubURL: 'githubURL',
    portfolioURL: 'portfolioURL',
    profilePicture: 'profilePicture',
    role: 'role'
};
export const CompanyScalarFieldEnum = {
    id: 'id',
    name: 'name',
    logo: 'logo',
    URL: 'URL'
};
export const JobsScalarFieldEnum = {
    id: 'id',
    hybridOrRemote: 'hybridOrRemote',
    jobQualification: 'jobQualification',
    fullTimeOrNot: 'fullTimeOrNot',
    location: 'location',
    datePosted: 'datePosted',
    jobTitle: 'jobTitle',
    jobDescription: 'jobDescription',
    jobAnchor: 'jobAnchor',
    companyID: 'companyID'
};
export const InstructionsScalarFieldEnum = {
    id: 'id',
    careersButton: 'careersButton',
    careersHoverButton: 'careersHoverButton',
    joinUsButton: 'joinUsButton',
    joinUsHoverButton: 'joinUsHoverButton',
    loadMoreButton: 'loadMoreButton',
    locationSelect: 'locationSelect',
    locationClickSelect: 'locationClickSelect',
    jobCategorySelect: 'jobCategorySelect',
    jobCategoryClickSelect: 'jobCategoryClickSelect',
    jobTypingInput: 'jobTypingInput',
    submitFormButton: 'submitFormButton',
    scrollToContainer: 'scrollToContainer',
    nextPageButton: 'nextPageButton',
    showMoreJobsOnPage: 'showMoreJobsOnPage',
    jobsContainer: 'jobsContainer',
    jobsContainerTitle: 'jobsContainerTitle',
    jobsContainerLocation: 'jobsContainerLocation',
    jobsWorkingDay: 'jobsWorkingDay',
    jobsWorkingType: 'jobsWorkingType',
    jobsDatePosted: 'jobsDatePosted',
    jobsContainerDesc: 'jobsContainerDesc',
    jobsContainerDetailsAnchor: 'jobsContainerDetailsAnchor',
    companyID: 'companyID'
};
export const SessionScalarFieldEnum = {
    id: 'id',
    sid: 'sid',
    data: 'data',
    expiresAt: 'expiresAt'
};
export const SortOrder = {
    asc: 'asc',
    desc: 'desc'
};
export const QueryMode = {
    default: 'default',
    insensitive: 'insensitive'
};
export const NullsOrder = {
    first: 'first',
    last: 'last'
};
//# sourceMappingURL=prismaNamespaceBrowser.js.map