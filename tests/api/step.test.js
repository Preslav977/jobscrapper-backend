...
import { createTestCompany, createTestSteps, createTestUser, } from "../helpersFunctions/helperFunctions.js";
    ...
    describe("[POST], /companies/steps", () => {
        ...
        it("user shouldn't create steps for company if the array is empty", async () => {
            ...
            const { body, header, status } = await request(app)
                ...
                .set("Authorization", `Bearer ${testUser.token}`)
                .send([]);
            expect(body.message).toEqual("Failed to creates steps for company! Is the array empty or the ID exists?");
            expect(header["content-type"]).toMatch(/json/);
