...
describe("Extracting text integration", async () => {
    it("should extract text from jobs", async () => {
        ...
        await mockPage.goto("http://127.0.0.1:5500/tests/test-fixtures/adastra.jobs.html");
    ...
    it("should return empty array if no jobs exists", async () => {
        ...
        const mockPage = await browser.newPage();
        await mockPage.goto("http://127.0.0.1:5500/tests/test-fixtures/no-jobs.html");
        const jobs = await extractJobsText(mockPage, instruction, 1);
        expect(jobs).toEqual([]);
    }, 20000);
...
//# sourceMappingURL=extractJobsText.test.js.map