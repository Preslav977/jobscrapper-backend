...
async function extractJobsText(
  ...
  try {
    ...
    console.error(
      `extractJobsText failed to scrap, instruction might have changed: ${error}`,
    );
...
async function extractJobsDetailsText(page: Page, instruction: Instructions) {
  ...
  try {
    ...
    console.error(
      `extractJobsDetailsText failed to scrap, instruction might have changed: ${error}`,
    );
    return { structuredText: "", rawHTML: "" };
  }
}
...
function parseMarkedUpText(rawText: string) {
  ...
  sections.forEach((section) => {
    ...
    if (header.match(/routine|responsibilities|tasks|daily|role/i)) {
      ...
      header.match(
        /technology stack|qualification|requirements|skills|requirements|unique|you have/i,
      )
    ) {
